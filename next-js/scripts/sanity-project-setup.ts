...
function upsertEnv(
  ...
  const updated = lines.map((line) => {
    ...
    if (!name) return line;
    seen.add(name);
    const publicName = name.replace(/^NEXT_PUBLIC_/, "PUBLIC_");
    const value = values[name] ?? values[publicName];
  ...
  for (const [name, value] of Object.entries(values)) {
    if (
      value !== undefined &&
      !seen.has(name) &&
      !seen.has(name.replace(/^PUBLIC_/, "NEXT_PUBLIC_"))
    ) {
...
async function main() {
  ...
  const envValues: Record<string, string | undefined> = {
    NEXT_PUBLIC_SANITY_PROJECT_ID: projectId || "abcd1234",
  ...
  log.title("Done");
  log.info("Next: npm run dev, then open http://localhost:3000/studio");
  if (projectId)
    log.info(
